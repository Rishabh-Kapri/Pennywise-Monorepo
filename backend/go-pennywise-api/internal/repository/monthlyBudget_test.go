...
func (ts *testSuite) insertDummyData(t *testing.T) {
	...
	dummyRows := [][]any{
		...
		{uuid.New(), "2025-07", ts.budgetID, ts.catID, 0.00, 0.00},    // activity: 0
		{uuid.New(), "2025-08", ts.budgetID, ts.catID, 0.00, -500.00}, // activity: -500
	}
...
func TestGetByCatIdAndMonth(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			monthlyBudget, err := ts.repo.GetByCatIdAndMonth(ts.ctx, nil, ts.budgetID, ts.catID, tc.month)
			log.Printf("%v %v", monthlyBudget, err)
		})
	}
}
...
func TestCreate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Execute operation
			err := ts.repo.Create(ts.ctx, nil, ts.budgetID, tc.monthlyBudget)
...
func TestUpdateCarryoverByCatIdAndMonth(t *testing.T) {
	testCases := []updateTestCase{
		{
			name:                "ValidLastMonthUpdate",
			description:         "No month exists after this",
			month:               "2025-08",
			amount:              500.00,
			expectError:         false,
			...
			expectedData: map[string]budgetData{
				"2025-05": {Budgeted: 1000.00, CarryoverBalance: 0.00},
				"2025-06": {Budgeted: 2500.00, CarryoverBalance: 0.00},
				"2025-07": {Budgeted: 0.00, CarryoverBalance: 0.00},
				"2025-08": {Budgeted: 0.00, CarryoverBalance: 0.00},
			},
		},
		{
			name:                "ValidMidMonthUpdate",
			description:         "",
			month:               "2025-07",
			amount:              -100.00,
			expectError:         false,
			...
			expectedData: map[string]budgetData{
				"2025-05": {Budgeted: 1000.00, CarryoverBalance: 0.00},
				"2025-06": {Budgeted: 2500.00, CarryoverBalance: 0.00},
				"2025-07": {Budgeted: 0.00, CarryoverBalance: -100.00},
				"2025-08": {Budgeted: 0.00, CarryoverBalance: -600.00},
			},
		},
		{
